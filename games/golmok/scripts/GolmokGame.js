...
let   ANIM_TEST_SLOW = 1;      // 총/마법 애니 속도 배수 (1=정상. 테스트 시 3 등으로 느리게 — 픽셀무브 디테일 확인용)
...
export default class GolmokGame extends Scene {
    constructor() {
        ...
        // 피격 타겟(허수아비 등, targets.csv 로드) + 미사일형 발사체
        this._targets  = [];   // [{ id, row, entity, hp, hitMs, hits, blend }]
        this._missiles = [];   // [{ x0w,y0,x1w,y1,t,ms,arcH,size,color,target }]
    }
    ...
    onEnter(engine) {
        ...
        this._pmBlend = null;                         // 픽셀무브 블렌드(idle↔사격 포즈 전환) 상태 { src, dst, t, ms, onDone }
        ...
        this._casting = false; this._castPhase = null; this._castFlip = false;
        this._waveActive = false; this._skillReleasePending = false; this._waveT = 0;
        this._loadSkill();           // 마법 스킬(G) 캐스팅 포즈 + 설정 로드 (skills.csv, ch01_magic.json)
        this._preload();
        ...
        if (!this._keyBound) {
            ...
            // 키보드 G = 마법 스킬(준비+루프+마무리) + 온스크린 버튼 CustomEvent
            window.addEventListener('keydown', (ev) => { if ((ev.key === 'g' || ev.key === 'G') && !ev.repeat) this._startSkill(); });
            window.addEventListener('keyup',   (ev) => { if (ev.key === 'g' || ev.key === 'G') this._endSkill(); });
            window.addEventListener('golmok:skill:start', () => this._startSkill());
            window.addEventListener('golmok:skill:end',   () => this._endSkill());

            // 더블탭(0.5초 이내 같은 방향키 2번) → 달리기 전환
            const MOVE_KEYS = { ArrowLeft: 'left', a: 'left', A: 'left', ArrowRight: 'right', d: 'right', D: 'right' };
            window.addEventListener('keydown', (ev) => {
                const dir = MOVE_KEYS[ev.key];
                if (!dir || ev.repeat) return;
                const t = performance.now();
                if (this._lastTapDir === dir && (t - (this._lastTapTime ?? -Infinity)) <= 500) {
                    this._runMode = true;
                }
    ...
    async _loadGun() {
        ...
        try {
            ...
            this._gunAnim.load({
                default: 'atk_active',   // 딱 두 동작(idle/사격)만 — 준비/회수 동작은 픽셀무브 전환으로 대체
                parameters: [],
                transitions: [],
            });
    ...
    // stateDef 상의 특정 상태(state)의 마지막 프레임 인덱스 — 픽셀무브의 도착(dst)/출발(src) 포즈로 사용
    _lastFrameIdx(entity, stateName) {
        const frames = entity?.asm?.states?.[stateName]?.frames;
        return frames?.length ? frames[frames.length - 1] : 0;
    }
    ...
    // ── 피격 타겟(허수아비 등) 로드 ────────────────────────────────────
    // targets.csv 각 행 → pixels/<assetCategory>/<asset>.json 페치 후 엔티티 추가(고정 배치).
    // (엔진=로직, 게임 데이터=CSV — 타겟 추가/이동/교체는 코드 수정 없이 CSV만 바꿔서)
    async _loadTargets() {
        const e = this.engine; const D = e.data;
        if (!D?.targets) return;
        for (const row of D.targets.all()) {
            try {
                const j = await fetch(`./pixels/${row.assetCategory}/${row.asset}.json`, { cache: 'no-store' }).then(r => r.ok ? r.json() : null);
                if (!j) { console.warn('[golmok] target asset not found:', row.asset); continue; }
                const pw = Number(row.pw) || j.width;
                const ph = Number(row.ph) || j.height;
                const ent = e.entities.add(row.id, {
                    x: Number(row.spawn_x) || 0, y: Number(row.spawn_y) || 0,
                    pw, ph, layer: 2, visible: true,
                    frames: j.frames, _palette: j.palette, stateDef: j.stateDef,
                });
                this._targets.push({
                    id: row.id, row, entity: ent,
                    hp: Number(row.hp) || 0, hitMs: Number(row.hit_ms) || 280,
                    hits: 0, blend: null,
                });
            } catch (err) { console.warn('[golmok] target load fail:', row.id, err); }
        }
    }

    // idle(첫 프레임) → 사격 준비 완성 포즈(atk_start 마지막 프레임)로 픽셀무브 전환.
    // 중간 동작은 픽셀무브 자체가 표현하므로 곧장 발사 루프(atk_active)로 진입.
    _startAttack() {
        if (this._attacking || this._casting || this._pmBlend || !this._gun || !this._gunAnim) return;
        const p = this._player;
        this._atkFlip = !!p?.flipX;            // 플레이어 시선 방향으로 사격 (총 기본은 왼쪽)
        this._gun.flipX = this._atkFlip;
        this._positionGun();
        const lastIdx = this._lastFrameIdx(this._gun, 'atk_start');
        const src = this._pixelsScreen(p, p?._frameIdx ?? 0, this._atkFlip);          // idle 포즈(첫 프레임)
        const dst = this._pixelsScreen(this._gun, lastIdx, this._atkFlip);            // 사격 준비 완성 포즈(마지막 프레임)
        if (p) p.visible = false;
        this._gun.visible = false;
        this._fireAcc = 110;   // 전환 끝나면 즉발
        this._startPixelMoveBlend(src, dst, 180 * ANIM_TEST_SLOW, () => {
            this._gun.asm.setState('atk_active'); this._gun.asm.restart();
            this._gun.visible = true;
            this._attacking = true;
        });
    }
    // 사격 → idle: 현재 사격 포즈에서 idle 포즈로 픽셀무브 역전환.
    _endFire() {
        const p = this._player;
        if (!this._attacking || this._pmBlend || !this._gun) return;
        const src = this._pixelsScreen(this._gun, this._gun.asm.getCurrentFrame(), this._atkFlip);  // 사격 포즈
        const dst = this._pixelsScreen(p, p?._frameIdx ?? 0, this._atkFlip);                        // idle 포즈
        this._gun.visible = false;
        this._attacking = false;
        this._startPixelMoveBlend(src, dst, 180 * ANIM_TEST_SLOW, () => {
            if (p) { p.visible = true; if (p.asm) { p.asm.setState('idle'); p.asm.restart(); } }
        });
    }

    // ── 마법 스킬(G) 캐스팅 포즈 로드 + 발 정합 ───────────────────────
    // skills.csv 1행(asset/assetCategory/state_*) → ch01_gun.json과 동일 포맷의 멀티프레임 pixelJSON.
    // 별도 'magic' 엔티티(layer2, 평소 숨김)로 추가. 캐스팅 시 플레이어를 숨기고 발 위치를 맞춰 스왑.
    async _loadSkill() {
        const p = this._player; const e = this.engine;
        const D = this.engine?.data;
        const row = D?.skills?.all()?.[0] || null;
        this._skillRow = row;
        if (!p || !e || !row?.asset) return;
        try {
            const cat = row.assetCategory || 'characters';
            const m = await fetch(`./pixels/${cat}/${row.asset}.json`, { cache: 'no-store' }).then(r => r.ok ? r.json() : null);
            if (!m) return;
            this._magic = e.entities.add('magic', {
                x: 0, y: 0, pw: m.width, ph: m.height, layer: 2, visible: false,
                frames: m.frames, _palette: m.palette, stateDef: m.stateDef,
            });
            if (ANIM_TEST_SLOW > 1 && this._magic.asm) {   // 테스트용 슬로우 (픽셀무브 디테일 확인)
                for (const k in this._magic.asm.states) this._magic.asm.states[k].fps /= ANIM_TEST_SLOW;
            }
            this._magicAnim = new AnimatorController(this._magic.asm);
            this._magicAnim.load({ default: row.state_loop || 'cast_loop', parameters: [], transitions: [] });
            const pa = this._feetAnchor(p._frames?.[0]?.pixels);
            const ma = this._feetAnchor(m.frames?.[0]?.pixels);
            this._magicAnchor = (pa && ma)
                ? { pcx: pa.cx, ppw: p.pw, pby: pa.by, mcx: ma.cx, mpw: this._magic.pw, mby: ma.by }
                : null;
        } catch (err) { console.warn('[golmok] magic load fail:', err); }
    }

    // 마법 스킬 시작 — idle(첫 프레임) → 캐스팅 완성 포즈(state_start의 마지막 프레임)로 픽셀무브 전환.
    // 중간 동작은 픽셀무브 자체가 표현하므로 곧장 루프(cast_loop)로 진입.
    _startSkill() {
        if (this._casting || this._attacking || this._pmBlend || !this._magic || !this._magicAnim || !this._skillRow) return;
        const row = this._skillRow, p = this._player;
        const startName = row.state_start || 'cast_start';
        const loopName = row.state_loop || 'cast_loop';
        this._castFlip = !!p?.flipX;
        this._magic.flipX = this._castFlip;
        this._positionSkill();
        const lastIdx = this._lastFrameIdx(this._magic, startName);
        const src = this._pixelsScreen(p, p?._frameIdx ?? 0, this._castFlip);          // idle 포즈(첫 프레임)
        const dst = this._pixelsScreen(this._magic, lastIdx, this._castFlip);          // 캐스팅 완성 포즈(마지막 프레임)
        if (p) p.visible = false;
        this._magic.visible = false;
        this._waveActive = false;
        this._waveT = 0;
        this._skillReleasePending = false;
        this._startPixelMoveBlend(src, dst, 180 * ANIM_TEST_SLOW, () => {
            this._magic.asm.setState(loopName); this._magic.asm.restart();
            this._magic.visible = true;
            this._casting = true;
            this._castPhase = 'loop';
            this._waveActive = true; this._waveT = 0;
        });
    }

    // G 키 떼면: 즉시 끊지 않고 현재 진행 중인 단계가 끝나면 마무리(cast_end) 단계로 넘어가도록 예약.
    _endSkill() {
        if (!this._casting) return;
        this._skillReleasePending = true;
    }

    // 마법 캐스팅 스프라이트를 플레이어 발치에 정합 (flipX 미러 고려)
    _positionSkill() {
        const p = this._player, a = this._magicAnchor, m = this._magic;
        if (!p || !m) return;
        if (a) {
            const pFeetX = this._castFlip ? (a.ppw - 1 - a.pcx) : a.pcx;
            const mFeetX = this._castFlip ? (a.mpw - 1 - a.mcx) : a.mcx;
            m.x = p.x + pFeetX - mFeetX;
            m.y = p.y + (a.pby - a.mby);
    ...
    _spawnShot() {
        ...
        // 미사일형 발사체 — 진행방향에 타겟이 있으면 그쪽으로 포물선 비행, 없으면 일정 거리 날아가 소멸.
        this._spawnMissile(m.x, m.y, dir, this._findTargetAhead(dir));
    }

    // 현재 캐릭터의 발사체 타입(projectiles.csv) 행 — characters.csv projectile_id 참조.
    _projectileRow() {
        const D = this.engine.data;
        const charId = this._player?.type || 'ch01_player';
        const chRow  = D?.characters?.byId('id', charId);
        const projId = chRow?.projectile_id || 'missile01';
        return D?.projectiles?.byId('id', projId);
    }

    // 사격 방향(dir: +1 오른쪽/-1 왼쪽)에 있는 가장 가까운(사거리 내) 피격 타겟
    _findTargetAhead(dir) {
        const p = this._player;
        if (!p || !this._targets.length) return null;
        const row   = this._projectileRow();
        const range = Number(row?.range) || 500;
        const px = p.x + p.pw / 2;
        let best = null, bestDist = Infinity;
        for (const tg of this._targets) {
            const te = tg.entity; if (!te || !te.visible) continue;
            const d = (te.x + te.pw / 2) - px;
            if (dir > 0 ? d <= 0 : d >= 0) continue;   // 반대 방향 — 제외
            const ad = Math.abs(d);
            if (ad < range && ad < bestDist) { best = tg; bestDist = ad; }
        }
        return best;
    }

    // 미사일(직선 투사체) 발사 — characters.csv→projectiles.csv 파라미터 사용.
    // 화면 끝까지 직선으로 빠르게 비행. 좌표는 월드 기준 저장(x0w/x1w) → 렌더 시 카메라로 화면좌표 환산.
    // 사거리 내 타겟이 있으면 비행 경로상 타겟 x를 지나는 시점(hitT)에 피격 처리(비행은 끝까지 계속).
    _spawnMissile(sx, sy, dir, target) {
        const row    = this._projectileRow();
        const speed  = Number(row?.speed)       || 1400;
        const size   = Number(row?.size)        || 4;
        const color  = row?.color               || '#fff2b0';
        const margin = Number(row?.edge_margin) || 40;

        const cam = this.engine.cameraX;
        const x0w = sx + cam, y0 = sy;
        const edgeScreenX = dir > 0 ? this.engine.gameWidth + margin : -margin;
        const x1w = edgeScreenX + cam;
        const y1  = y0;   // 직선(수평) — 높이 변화 없음

        const dist = Math.abs(x1w - x0w);
        const ms = Math.max(40, (dist / speed) * 1000);

        let hitT = null;
        if (target) {
            const te = target.entity;
            const tx = te.x + te.pw / 2;
            const f = (tx - x0w) / (x1w - x0w);
            if (f > 0 && f < 1) hitT = f;
        }
        this._missiles.push({ x0w, y0, x1w, y1, t: 0, ms, size, color, target, hitT, hitDone: false });
    }
    ...
    _updateMissiles(dt) {
        for (let i = this._missiles.length - 1; i >= 0; i--) {
            ...
            m.t += dt / m.ms;
            if (m.target && !m.hitDone && m.hitT != null && m.t >= m.hitT) {
                m.hitDone = true;
                this._hitTarget(m.target);
            }
            if (m.t >= 1) this._missiles.splice(i, 1);
        }
    }
    ...
    _renderMissiles(ctx) {
        ...
        for (const m of this._missiles) {
            ...
            const x = (m.x0w + (m.x1w - m.x0w) * t) - cam;
            const y = m.y0;
            const pt = Math.max(0, t - 0.07);
            const tx = (m.x0w + (m.x1w - m.x0w) * pt) - cam;
            ctx.globalAlpha = 0.55;
            ctx.strokeStyle = m.color;
            ctx.lineWidth = Math.max(1, m.size * 0.6);
            ctx.beginPath(); ctx.moveTo(tx, y); ctx.lineTo(x, y); ctx.stroke();
            ctx.globalAlpha = 1;
            ctx.fillStyle = m.color;
            ctx.fillRect((x - m.size / 2) | 0, (y - m.size / 2) | 0, m.size, m.size);
        }
        ctx.restore();
    }

    // 타겟 피격 — 픽셀무브로 흩어졌다가(scatter) 다시 모인다(reform). idle↔사격 전환과 동일 톤.
    _hitTarget(target) {
        if (!target?.entity || target.blend) return;
        target.hits++;
        const ent = target.entity;
        const frameIdx = ent.asm ? ent.asm.getCurrentFrame() : (ent._frameIdx ?? 0);
        const src = this._pixelsScreen(ent, frameIdx, false);
        const sMin = Number(target.row?.scatter_min) || 18;
        const sMax = Number(target.row?.scatter_max) || 64;
        const scattered = src.map(px => {
            const ang = Math.random() * Math.PI * 2;
            const dist = sMin + Math.random() * Math.max(0, sMax - sMin);
            return [px[0] + Math.cos(ang) * dist, px[1] + Math.sin(ang) * dist, px[2], px[3], px[4]];
        });
        ent.visible = false;
        const ms = target.hitMs || 280;
        target.blend = { src, dst: scattered, t: 0, ms, onDone: () => {
            target.blend = { src: scattered, dst: src, t: 0, ms, onDone: () => {
                ent.visible = true;
                target.blend = null;
            }};
        }};
    ...
    // 픽셀무브 테스트 초기화 — 32px 네모 둘레의 36개 점 ↔ 반경 18px 동그라미 둘레의 36개 점 (1:1 매핑)
    _initPixelMoveTest() {
        const N = 36, side = 32, r = side * 0.56;
        const square = [], circle = [];
        const perim = side * 4;
        for (let i = 0; i < N; i++) {
            const d = (i / N) * perim;
            let x, y;
            if (d < side)            { x = d;                    y = 0; }
            else if (d < side * 2)   { x = side;                 y = d - side; }
            else if (d < side * 3)   { x = side - (d - side*2);  y = side; }
            else                     { x = 0;                    y = side - (d - side*3); }
            square.push([x - side / 2, y - side / 2]);
            const ang = (i / N) * Math.PI * 2;
            circle.push([Math.cos(ang) * r, Math.sin(ang) * r]);
        }
        this._pmTest = { time: 0, square, circle };
    }

    _updatePixelMoveTest(dt) {
    ...
    _renderPixelMoveTest(ctx) {
        ...
        if (!T || !p) return;
        const phase = (Math.sin(T.time * 1.6) + 1) / 2;     // 0↔1 핑퐁
        const morph = phase * phase * (3 - 2 * phase);       // smoothstep
        const spin = phase * Math.PI * 2;
        const dir = p.flipX ? 1 : -1;
        const cx = (p.x - this.engine.cameraX) + p.pw / 2 + dir * 40;
        const cy = p.y + p.ph / 2;
        const cos = Math.cos(spin), sin = Math.sin(spin);
        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        for (let i = 0; i < T.square.length; i++) {
            const a = T.square[i], b = T.circle[i];
            const x = a[0] + (b[0] - a[0]) * morph;
            const y = a[1] + (b[1] - a[1]) * morph;
            const rx = x * cos - y * sin;
            const ry = x * sin + y * cos;
            ctx.fillStyle = (i % 2 === 0) ? 'rgb(255,110,210)' : 'rgb(110,200,255)';
            ctx.fillRect((cx + rx) | 0, (cy + ry) | 0, 3, 3);
        }
        ctx.restore();
    }
    ...
    // 픽셀무브 블렌드 시작 — src 포즈가 픽셀 단위로 흩어져 dst 포즈로 모여든다 (idle ↔ 사격).
    _startPixelMoveBlend(src, dst, ms, onDone) {
        this._pmBlend = { src: src || [], dst: dst || [], t: 0, ms: Math.max(1, ms), onDone };
    }

    // 픽셀무브 전환 렌더 — src→dst 픽셀이 위치/색을 보간하며, 전환 정점에서 핑크/시안 글로우로 번쩍인다.
    // (플레이어 idle↔사격 전환, 타겟 피격 흩어짐↔재조립 등 — blend={src,dst,t,ms} 형태면 공용으로 사용)
    _renderPixelMove(ctx, b) {
        if (!b) return;
        const t = b.t < 0 ? 0 : b.t > 1 ? 1 : b.t;
        const ease = t * t * (3 - 2 * t);
        const swirl = Math.sin(ease * Math.PI);        // 0→1→0 (전환 정점에서 최대)
        const spin = ease * Math.PI * 2;
        const src = b.src, dst = b.dst;
        const n = Math.max(src.length, dst.length, 1);
        ctx.save();
        for (let i = 0; i < n; i++) {
            const s = src.length ? src[i % src.length] : null;
            const d = dst.length ? dst[i % dst.length] : null;
            let x, y, r, g, bl, a = 1;
            if (s && d) {
                x = s[0] + (d[0] - s[0]) * ease;
                y = s[1] + (d[1] - s[1]) * ease;
                ...
                x = s[0]; y = s[1]; r = s[2]; g = s[3]; bl = s[4];
                a = 1 - ease;
            } else if (d) {
                x = d[0]; y = d[1]; r = d[2]; g = d[3]; bl = d[4];
                a = ease;
            } else continue;

            // 픽셀무브 소용돌이 변위 + 회전
            const ang = i * 0.61803 + spin;
            const rad = swirl * 10;
            x += Math.cos(ang) * rad;
            y += Math.sin(ang) * rad;
            ...
            if (swirl > 0.05) {
                ctx.globalCompositeOperation = 'lighter';
                ctx.fillStyle = (i % 2 === 0) ? `rgba(255,110,210,${swirl})` : `rgba(110,200,255,${swirl})`;
    ...
    // 플레이어 idle↔사격 전환 (this._pmBlend) — _renderPixelMove 공용 함수에 위임.
    _renderPixelMoveBlend(ctx) {
    ...
    // 마법 스킬 루프 단계 — 화면 전체에 유기적으로 움직이는 모래-픽셀 파도(핑크/시안 글로우).
    // 색은 skills.csv wave_color1/2 (기본 _renderPixelMove의 핑크/시안과 동일 톤).
    _renderMagicWave(ctx) {
        if (!this._waveActive) return;
        const row = this._skillRow;
        const c1 = this._hexToRgb(row?.wave_color1) || { r: 255, g: 110, b: 210 };
        const c2 = this._hexToRgb(row?.wave_color2) || { r: 110, g: 200, b: 255 };
        const dens = Number(row?.wave_density) || 0.35;
        const W = this.engine.gameWidth, H = this.engine.gameHeight;
        const G = 6;
        const t = this._waveT / 1000;
        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        for (let y = 0; y < H; y += G) {
            for (let x = 0; x < W; x += G) {
                const n = sandHash(x / G, y / G);
                const wave = (Math.sin(x * 0.018 + y * 0.012 + t * 3.2 + n * Math.PI * 2) + 1) / 2;
                if (wave < (1 - dens)) continue;
                const strength = (wave - (1 - dens)) / dens;
                const r = c1.r + (c2.r - c1.r) * n;
                const g = c1.g + (c2.g - c1.g) * n;
                const b = c1.b + (c2.b - c1.b) * n;
    ...
    onUpdate(now, dt, input) {
        ...
        // 입력 dx 저장(잠금 전) — 달리기 블렌드 중 speed 파라미터 유지에 사용
        const rawDx = dx;
        // 이동 잠금: 공격·블렌드·멈춤(run_stop) 중 무시. 달리기 전환 블렌드(_runTransition)는 speed만 유지, 위치는 잠금.
        const stopping = player.asm?.current === 'run_stop';
        if (this._attacking || this._casting || this._pmBlend || stopping) { dx = 0; this._targetX = null; }

        // 달리기 — keydown 핸들러에서 더블탭 감지 시 _runMode=true.
        // 달리기 전환 블렌드 중(_pmBlend.isRunTransition)엔 리셋 안 함 — 블렌드 끝날 때까지 달리기 유지.
        if (dx === 0 && !this._pmBlend?.isRunTransition) this._runMode = false;
        const isRunning = !!this._runMode;
        ...
        // 애니: 파라미터 세팅 전 현재 상태 스냅샷 → 전환 감지(달리기 시작/정지 픽셀무브용)
        const _prevRunState = player.asm?.current;
        const _prevRunFrame = player.asm?.getCurrentFrame() ?? player._frameIdx ?? 0;
        const _prevRunFlip  = player.flipX;
        // speed는 rawDx 기준 — 달리기 전환 블렌드 중 위치는 잠금이지만 speed=1 유지해 run→run_stop 전환 방지
        if (this._anim) { this._anim.setParam('speed', rawDx !== 0 ? 1 : 0); this._anim.update(); }
        else            { player.setState(rawDx !== 0 ? 'walk' : 'idle'); }
        // 달리기 시작(→run) / 정지(→run_stop) 전환 시 빠른 픽셀무브
        const _curRunState = player.asm?.current;
        if (this._anim && !this._pmBlend && !this._attacking && !this._casting &&
            _curRunState !== _prevRunState &&
            (_curRunState === 'run' || _curRunState === 'run_stop')) {
            const src = this._pixelsScreen(player, _prevRunFrame, _prevRunFlip);
            const dst = this._pixelsScreen(player, player.asm.getCurrentFrame(), player.flipX);
            player.visible = false;
            this._startPixelMoveBlend(src, dst, 90, () => { player.visible = true; });
            if (_curRunState === 'run') this._pmBlend.isRunTransition = true;  // 달리기 시작 블렌드 — _runMode 리셋 방지
        }
        player.y = this._groundY;                                            // 하드코딩된 GROUND_Y 대신 동적 바닥 높이 적용

        // 픽셀무브 전환 진행 — idle ↔ 사격 포즈 모핑.
        if (this._pmBlend) {
            this._pmBlend.t += dt / this._pmBlend.ms;
            if (this._pmBlend.t >= 1) {
                const b = this._pmBlend; this._pmBlend = null;
                if (b.onDone) b.onDone();
            }
        }

        // 공격(총): 발치 정합 + 발사 임팩트 FX. 딱 두 동작(idle/사격) — 전환은 픽셀무브가 담당.
        ...
        // 마법 스킬(G) — 루프(cast_loop, 화면 모래파도) 유지. 키를 떼면(_skillReleasePending)
        // 캐스팅 포즈(첫 프레임) → idle(마지막 프레임)로 픽셀무브 역전환. 중간 동작 없음 — 픽셀무브가 표현.
        if (this._casting && this._magic && this._magicAnim) {
            this._positionSkill();
            this._magicAnim.update();
            const asm = this._magic.asm;
            if (this._castPhase === 'loop') {
                this._waveT += dt;
                if (this._skillReleasePending) {
                    const src = this._pixelsScreen(this._magic, asm.getCurrentFrame(), this._castFlip); // 캐스팅 포즈(첫 프레임)
                    const dst = this._pixelsScreen(this._player, this._player?._frameIdx ?? 0, this._castFlip); // idle 포즈(마지막 프레임)
                    this._magic.visible = false;
                    this._castPhase = null;
                    this._waveActive = false;
                    this._startPixelMoveBlend(src, dst, 180 * ANIM_TEST_SLOW, () => {
                        const p = this._player;
                        if (p) { p.visible = true; if (p.asm) { p.asm.setState('idle'); p.asm.restart(); } }
                        this._casting = false; this._skillReleasePending = false;
                    });
                }
            }
        }
        ...
        for (const tg of this._targets) {
            if (!tg.blend) continue;
            tg.blend.t += dt / tg.blend.ms;
            if (tg.blend.t >= 1) {
                const b = tg.blend; tg.blend = null;
                if (b.onDone) b.onDone();
            }
        }
    ...
    onPostRender(canvas) {
        ...
        if (this._pmBlend) {
            this._renderPixelMoveBlend(ctx);
        } else if (gunActive) {
        ...
        for (const tg of this._targets) {
            if (tg.blend) this._renderPixelMove(ctx, tg.blend);
        }
